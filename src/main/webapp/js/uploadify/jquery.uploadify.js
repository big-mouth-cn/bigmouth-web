		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					width           : 79,                // The width of the browse button
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).delay(1000 + 100 * delay).fadeOut(100, function() {
								$(this).remove();
							});
						});
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).delay(1000 + 100 * n).fadeOut(100, function() {
								$(this).remove();
							});
						}
					}
					...
					$item.delay(1000).fadeOut(100, function() {
						$(this).remove();
					});
				}
			});
...
//			}
			...
			if ($.inArray('onSelect', settings.overrideEvents) < 0) {
				...
				var obj = $('#' + settings.queueID);
				obj.append($.tmpl($itemTemplate, itemData));
			}
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n您上传文件的次数超过了限制 (' + errorMsg + ')。';
						} else {
							this.queueData.errorMsg += '您最多还能上传 (' + settings.queueSizeLimit + ')个文件。';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n选择的文件 "' + file.name + '" 超过了大小限制 (' + settings.fileSizeLimit + ').';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n选择的文件 "' + file.name + '" 为空.';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n选择的文件 "' + file.name + '" 是不被接受的文件类型 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('一个文件的名称 "' + file.name + '" 已经存在于服务器上。\n您要取代现有的文件吗？');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
