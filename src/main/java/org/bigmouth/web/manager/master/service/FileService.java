...
import org.bigmouth.web.manager.utils.ImageUtils;
...
public class FileService {
    ...
    public void store(Photo photo, File snapshot, String imgFileName, int minSize) throws Exception {
        String saveHolder = getFileSaveHolder();
        String browseURL = getFileBrowseURL();
        File file = new File(saveHolder);
        if (!file.exists()) {
        ...
        String origFileName = getFileName(imgFileName);
        String thumbnailFileName = getFileName(imgFileName);
        File img = new File(saveHolder + origFileName);
        FileUtils.copy(snapshot, img);
        int[] wh = ImageUtils.compressImageMin(img.getPath(), saveHolder + thumbnailFileName, minSize);
        /*
         * Exif 
        ...
        */
        String origPath = browseURL + origFileName;
        String thumbnailPath = browseURL + thumbnailFileName;
        photo.setPhotoUri(origPath);
        photo.setThumbnailUri(thumbnailPath);
        photo.setThumbnailWidth(wh[0]);
        photo.setThumbnailHeight(wh[1]);
    }
